...
# ---------- Page config ----------
st.set_page_config(
    page_title="CV Screening System",
    page_icon="🧠",
    layout="wide",
...
EDUCATION_OPTIONS = ["High School", "Bachelor's", "Master's", "PhD", "Any"]
EMPLOYMENT_OPTIONS = ["Full-time", "Part-time", "Remote", "Hybrid", "Contract"]


# ---------- Sidebar navigation ----------
def sidebar_nav():
    st.sidebar.title("🧠 CV Screener")
    st.sidebar.markdown("---")
    ...
    for key, label in nav_items.items():
        ...
        if st.sidebar.button(label, use_container_width=True, type=button_type, key=f"nav_{key}"):
            st.session_state.page = key
            st.rerun()

    st.sidebar.markdown("---")
    st.sidebar.caption(f"📁 {database.get_history_count()} candidates analyzed so far")


# ---------- PAGE 1: Post Job & Upload ----------
def page_upload():
    st.title("📋 Job & Upload CVs")
    st.caption("Fill in the role details, upload candidate CVs, and let AI screen them for you.")
...
def page_results():
    ...
    if not results:
        ui.render_empty_state(
            "🗂️",
            "No results yet",
            "Head over to 'Post Job & Upload CVs' to analyze your first batch of candidates.",
        )
        return
...
def page_history():
    ...
    with col2:
        search_field = st.selectbox("Search in", ["Both", "Job Title", "Candidate Name"])
